...
l'exploració, manipulació, visualització i anàlisi de dades comença amb la càrrega de les dades des de diferents formats de fitxers. En aquesta activitat veurem com llegir fitxers de tipus `CSV` i `Excel`, carregar-los en un `DataFrame` i tornar-los a guardar en un altre fitxer.

Farem servir les biblioteques [`readr`](https://readr.tidyverse.org) i [`readxl`](https://readxl.tidyverse.org), que formen part de la col·lecció de paquets [`tidyverse`](https://tidyverse.org/).
